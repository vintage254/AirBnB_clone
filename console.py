...
class HBNBCommand(cmd.Cmd):
    ...
    def default(self, arg):
        ...
        match_mod = re.search(r"\.", arg)
        if match_mod is not None:
            argl = [arg[:match_mod.span()[0]], arg[match_mod.span()[1]:]]
            match_mod = re.search(r"\((.*?)\)", argl[1])
            if match_mod is not None:
                command = [argl[1][:match_mod.span()[0]], match_mod.group()[1:-1]]
